...
For public HTTPS on the Daleel server, set the `dalilee.uk` apex (`@`) A record to `135.181.117.16` in Cloudflare DNS. Keep the record **DNS only** while issuing the origin certificate. This DNS change happens in Cloudflare, not in this repository: pushing a Compose or README change does not update the public A record. Confirm that `dig +short @1.1.1.1 dalilee.uk A` returns `135.181.117.16`, then run `sudo certbot --nginx -d dalilee.uk` on that server and verify `curl -I https://dalilee.uk/`. The HTTP-01 challenge must reach port 80 on this same server; the discovery script's final status line alone is not evidence that Certbot succeeded. If DNS instead points to a separate edge server, issue the certificate there and route requests to the Daleel backend on port `18627`. The Docker proxy labels identify the backend service but do not change DNS or configure a separate edge server.
